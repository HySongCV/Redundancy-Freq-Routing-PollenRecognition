# Note:
#   The original configuration names are retained for compatibility:
#   PARTIAL_HEAT corresponds to Partial Heat Routing (PHR).
#   FP / freq_pruning corresponds to Frequency-domain Regularization (FDR).
...
def build_vheat_from_config(config, num_classes):
    ...
    For compatibility with the original experimental code, the configuration
    names are retained:
    
        MODEL.PARTIAL_HEAT -> Partial Heat Routing (PHR) in the paper
        MODEL.FP           -> Frequency-domain Regularization (FDR) in the paper
    
    The optional MODEL.REDUNDANCY.PHR / MODEL.REDUNDANCY.FDR style is also
    supported for future migration.
    """
