...
class CPD1CSVDataset(Dataset):
    def __init__(self, root, csv_path, transform=None, is_train=True):
        """
        CSV-based dataset loader for CPD-1 (Cretan Pollen Dataset v1).

        Expected CSV format:
            image,label

        Required columns:
            image: relative image path under the dataset root. PNG images are supported.

        Optional columns:
            label: integer class index in [0, 19] for the 20 CPD-1 classes.
        ...
        self.annotations["image"] = self.annotations["image"].str.replace("\\", "/", regex=False)
        self.annotations["image"] = self.annotations["image"].str.replace(r"/+", "/", regex=True)
        ...
        if self.has_label:
            ...
            if not invalid_labels.empty:
                raise ValueError(
                    ...
                    "for the 20 CPD-1 classes. Examples of invalid rows:\n"
